...
void ButterflyTextureComputeShaderManager::Dispatch(UTextureRenderTarget2D* target)
{
	ENQUEUE_RENDER_COMMAND(WaveComputeCmd)([target](FRHICommandListImmediate& rhiCmdList)
	{
		...
		rdgBuilder.QueueTextureExtraction(outTextureRef, &pooledRenderTarget);
		rdgBuilder.Execute();
		
		rhiCmdList.CopyTexture(pooledRenderTarget->GetRHI(), target->GetRenderTargetResource()->TextureRHI, FRHICopyTextureInfo());
	});
}


void ButterflyTextureComputeShaderManager::UpdateParameters(FParams& drawParameters)
{
	//mLastParams = drawParameters;
}


ButterflyTextureComputeShaderManager* ButterflyTextureComputeShaderManager::mSingleton;